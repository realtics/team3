...
public class CarEffectsController : MonoBehaviour
{
    ...
    public void AdjustEngineSound(float curSpeed, CarController.CarState carState)
    {
        float temp = 0.5f;
        ...
        if (carState != CarController.CarState.controlledByPlayer)
        {
    ...
    void EnableParticle()
    {
        if (carManager.carDamage.hp <= 0)
        {
            fireParticle.SetActive(false);
            explosionParticle.SetActive(true);
        }
        else if (carManager.carDamage.hp < 30)
        {
