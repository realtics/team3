...
public class CarControllAi : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (carManager.carController.carState != CarController.CarState.controlledByAi)
            return;
    ...
    public void SetAiMaxSpeedMultiplier()
    {
        switch (aiState)
        {
            case AiState.normal:
    ...
    void CarMoveAI()
    {
        ...
        {
            targetSpeed = Mathf.Clamp(distToObstacle - 1, 0, 1) * carManager.carController.maxSpeed * maxSpdMultiplier * carManager.carDamage.maxSpdMultiplier;
        }

        targetSpeed *= (1 - (Mathf.Abs(h) / 2f));

        if (targetSpeed < carManager.carController.curSpeed)
        {
            v = -4;
        }
        else if (targetSpeed * 0.9f > carManager.carController.curSpeed)
        {
            v = 1;
        }
        else
        ...
        carManager.carController.AiInput(h, v);
        carManager.carController.MoveCarByInput();
    }
